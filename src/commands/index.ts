...
import type { CortexExplorerProvider, CortexNode } from '../tree/explorer';
...
export function registerCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cortex.nexus.initialize', () =>
      cmdInitialize(nexus),
    ),
    vscode.commands.registerCommand('cortex.nexus.switch', () =>
      nexus.setActiveByPick(),
    ),
    vscode.commands.registerCommand('cortex.nexus.openOrSwitch', () =>
      cmdOpenOrSwitch(nexus),
    ),
    vscode.commands.registerCommand('cortex.tree.refresh', () =>
      explorer.refresh(),
    ),
    vscode.commands.registerCommand('cortex.tree.openInReader', (nodeOrUri: CortexNode | vscode.Uri) => {
      // Item click passes vscode.Uri (via item.command arguments); context menu passes CortexNode
      const uri = nodeOrUri instanceof vscode.Uri
        ? nodeOrUri
        : (nodeOrUri.indexUri ?? nodeOrUri.uri);
      reader.open(uri);
    }),
    vscode.commands.registerCommand('cortex.tree.openSource', (node: { uri: vscode.Uri; indexUri?: vscode.Uri }) => {
