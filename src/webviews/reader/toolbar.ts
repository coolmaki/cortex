...
export function renderToolbar(
    ...
    const backBtn = btn(ICONS.back, "Back", true, onBack);
    const forwardBtn = btn(ICONS.forward, "Forward", true, onForward);
    const reloadBtn = btn(ICONS.reload, "Reload", false, () => post({ type: "reload" }));
    const editBtn = btn(ICONS.edit, "Edit Source", false, () => post({ type: "openSource" }));

    const pathEl = document.createElement("span");
    pathEl.className = "toolbar-path";

    toolbar.append(backBtn, forwardBtn, reloadBtn, editBtn, pathEl);
