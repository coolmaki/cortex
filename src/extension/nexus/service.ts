...
export class NexusService implements vscode.Disposable {
    ...
    private readonly _onDidChangeActive = new vscode.EventEmitter<
    ...
    async setActiveByPick(): Promise<void> {
        ...
        if (picked) {
            await this.setActive(picked.folder);
        }
    }
    ...
    private registerWorkspaceFolderListener(): void {
        this.disposables.push(vscode.workspace.onDidChangeWorkspaceFolders(() => this.rescan()));
    }
