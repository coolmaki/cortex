...
export class CortexExplorerProvider implements vscode.TreeDataProvider<CortexNode> {
  private readonly _onDidChangeTreeData = new vscode.EventEmitter<CortexNode | undefined | null | void>();
  ...
  getTreeItem(node: CortexNode): vscode.TreeItem {
    ...
    if (node.kind === 'file') {
      ...
      item.command = { command: 'cortex.tree.openInReader', title: 'Open in Reader', arguments: [node.uri] };
      item.tooltip = node.uri.fsPath;
    } else {
      item.contextValue = node.indexUri ? 'cortexFolderWithIndex' : 'cortexFolder';
      if (node.indexUri) {
        item.command = { command: 'cortex.tree.openInReader', title: 'Open in Reader', arguments: [node.indexUri] };
  ...
  async getChildren(parent?: CortexNode): Promise<CortexNode[]> {
    ...
    if (!active) return [this.noNexusNode()];
  ...
  private async buildChildren(
    ...
    for (const [name, type] of entries) {
      ...
      if (this.shouldSkip(name, uri)) continue;
      // Skip the index file that was merged into the parent folder node
      if (parentIndexUri && uri.toString() === parentIndexUri.toString()) continue;
  ...
  private async buildFolderNode(
    ...
    for (const indexName of INDEX_NAMES) {
      ...
      const indexRelPath = path.join(relPath, indexName);
      if (this.ignoreService?.isIgnored(candidate)) continue;
  ...
  private async folderHasVisibleContent(
    ...
    if (indexUri) return true; // index file is enough
    ...
    for (const [name, type] of entries) {
      ...
      if (this.shouldSkip(name, childUri)) continue;
      if (type === vscode.FileType.File && name.endsWith('.md')) {
        // Don't re-check index files (they'd be counted again)
        if (INDEX_NAMES.includes(name)) continue;
        const title = await this.frontmatter.getTitle(childUri);
        if (title) return true;
      } else if (type === vscode.FileType.Directory) {
        if (await this.folderHasVisibleContent(childUri, childRel, undefined)) return true;
      }
    }
    return false;
  }
  ...
  private shouldSkip(name: string, absUri: vscode.Uri): boolean {
    ...
    if (this.ignoreService?.isIgnored(absUri)) return true;
    return false;
  }
